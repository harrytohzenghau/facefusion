...
const AnimationController = {
  ...
  async download(req, res) {
    ...
    try {
      const videoPath = path.join(__dirname, '..', 'uploads', `${videoId}_sd.mp4`);
  ...
  async getAllVideos(req, res) {
    try {
      const userId = req.user.id;
      const videos = await AnimationJob.find({ user_id: userId, status: 'completed' });
      res.json(videos);
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  ...
  async generateExpression(req, res) {
    try {
      ...
      const response = await axios.post('http://20.205.137.58:8001/animate/', formData, {
  ...
  async generateLipSync(req, res) {
    try {
      ...
      const response = await axios.post('http://20.205.137.58:8000/generate/', formData, {
        headers: formData.getHeaders(),
      });
  ...
  async textToSpeech(req, res) {
    try {
      ...
      const response = await axios.post('http://20.205.137.58:8002/textToSpeech/', formData, {
        headers: formData.getHeaders(),
      });
